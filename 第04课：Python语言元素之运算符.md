...
>**说明**：上面代码中使用了 f-strings 来格式化字符串。f-strings 是一种新的字符串格式化方式，由 f 和 {} 组成，在 {} 中可以使用变量名和格式说明符来指定字符串中需要被替换的部分和如何格式化变量。例如，这里的第一个 f 表示这是一个 f-string，第一个 {} 中的 `f:.1f` 表示对变量 `f` 进行格式化，其中 `.1f` 表示保留一位小数（浮点数），第二个 {} 中的 `c:.1f` 也是类似的。
